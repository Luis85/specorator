...
- First run: every provider ships `enabled: false`, so a fresh install opens the **Setup view** (`src/features/onboarding/`) once — it probes each provider's CLI through that provider's own `ProviderCliResolver`, offers a shell-free in-app install for what is missing (`ProviderRegistration.cliInstall`), and confirms the vault folders, chat defaults, and placement. Gated on `onboardingAutoOpened` (written when the auto-open fires, so however the user closes the tab it never re-ambushes them) + `firstRunDismissed` + no enabled provider, and run from a failure-tolerant continuation after deferred onload so a broken provider init cannot suppress it.
...
| **features/teamChat** | MS-Teams-style 1:1 agent DM surface (Vue 3 island) | Main-area `VIEW_TYPE_TEAM_CHAT` view (`users` ribbon, `open-team-chat` command) reusing the untouched chat engine as a SECOND host: a roster rail + single DM pane + agent identity/edited-files top bar, over its OWN per-leaf `TabManager` + Pinia. Each DM is a `Conversation` bound to a roster agent (`boundAgentId`, `surface: 'team-chat'`) on the agent's own provider+model; `getAllViews()` enumerates both hosts while `getView()` stays sidebar-scoped. The human is the sole orchestrator (increment 1, 1:1 only). See [`src/features/teamChat/CLAUDE.md`](src/features/teamChat/CLAUDE.md) |
...
| **features/marketplace** | Remote catalog browse + install (Vue 3 island) | Dedicated `VIEW_TYPE_MARKETPLACE` view (`store` ribbon, `open-marketplace` command) that fetches the `specorator-marketplace` catalog and installs loops/agents/templates/quick-actions into the vault Library, plus multi-file **skills** into a chosen provider skill root (Claude/Codex/Cursor) at project or user scope. An item may declare `requires` — a **package** of catalog ids installed with it (the Project Manager agent brings the project-artifact skills it works through), written dependencies-first, with an agent's skill dependencies bound to it on install. Network is **opt-in** (`marketplaceNetworkEnabled`, off by default), fetched over Obsidian `requestUrl`, SSRF-guarded by `assertSafeRemoteUrl` and constrained under the configured base URL (overridable via `marketplaceSourceUrl` for forks/mirrors), with a cache fallback at `.specorator/cache/marketplace-index.json`. Storefront UX: a primary category tab nav (Home + per-type tabs with counts), a sectioned Home landing, a responsive card grid, and an in-island detail/preview view (install gated behind it), with chrome-first skeleton loading — over the reused Library list/toolbar (`useLibraryList` + `LibraryToolbar`). Replaces the deleted bundled starter presets — ADR 0007. See [`src/features/marketplace/CLAUDE.md`](src/features/marketplace/CLAUDE.md) |
| **features/onboarding** | First-run guided setup (Vue 3 island) | Dedicated `VIEW_TYPE_ONBOARDING` view in the **main area** (no ribbon; `open-setup-guide` command + a Settings → General banner button) that opens itself **once** per vault. Five steps: **Providers** (probes each provider's CLI through the provider's own `ProviderCliResolver`, detected ones first, with an in-app CLI install), **Defaults** (model / the stated tool-approval default / auto-titles), **Folders** (creates the four Agent Board folders + Quick Actions), **Workspace** (chat placement, tab cap), **Marketplace** (network opt-in). Install runs a provider-declared static `argv` with **no shell** (`ProviderRegistration.cliInstall`; piped-script installs are copy-only), behind an explicit per-run confirm, with streamed output, cancel, timeout, and an automatic re-probe. Auto-open is gated by `onboardingAutoOpened` (written when it fires, so the tab's own X can't cause a re-ambush) plus `firstRunDismissed` + "no provider enabled", and runs from a failure-tolerant continuation after deferred onload so a broken provider init can't suppress it. See [`src/features/onboarding/CLAUDE.md`](src/features/onboarding/CLAUDE.md) |
...
| `.specorator/cache/marketplace-index.json` | Cached Marketplace catalog manifest (schema-versioned) for offline browse; refreshed on each successful fetch |
