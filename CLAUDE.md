...
| **features/marketplace** | Remote catalog browse + install (Vue 3 island) | Dedicated `VIEW_TYPE_MARKETPLACE` view (`store` ribbon, `open-marketplace` command) that fetches the `specorator-marketplace` catalog and installs loops/agents/templates/quick-actions into the vault Library (skills deferred). Network is **opt-in** (`marketplaceNetworkEnabled`, off by default), fetched over Obsidian `requestUrl`, SSRF-guarded by `assertSafeRemoteUrl` and constrained under the configured base URL (overridable via `marketplaceSourceUrl` for forks/mirrors), with a cache fallback at `.specorator/cache/marketplace-index.json`. Reuses Library list/toolbar components. Replaces the deleted bundled starter presets — ADR 0007. See [`src/features/marketplace/CLAUDE.md`](src/features/marketplace/CLAUDE.md) |
