...
| **providers/cursor** | Cursor Agent adaptor over first-party ACP (`agent acp`, not the retired `cursor-agent` stream-json CLI path) | Runtime (`AcpClientConnection` session lifecycle over the shared `src/providers/acp/` stack), `cursorAcpExtensions` dialect handlers (blocking `cursor/ask_question`, `cursor/create_plan`, `cursor/task`; `cursor/update_todos` notifications), tool-name normalization, prompt encoding, JSONL history hydration, settings reconciliation, plan-path conventions, settings UI |
