...
| **providers/cursor** | Cursor Agent adaptor over first-party ACP (`agent acp`, not the retired `cursor-agent` stream-json CLI path) | Runtime (`AcpClientConnection` session lifecycle over the shared `src/providers/acp/` stack), `cursorAcpExtensions` dialect handlers (blocking `cursor/ask_question`, `cursor/create_plan`, `cursor/task`, `cursor/generate_image`, `cursor/update_todos` — the last also registered as a notification for either frame shape), tool-name normalization, prompt encoding, JSONL history hydration, settings reconciliation, plan-path conventions, settings UI. See [`src/providers/cursor/CLAUDE.md`](src/providers/cursor/CLAUDE.md) |
