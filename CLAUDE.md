...
- Shared transport: `src/providers/acp/` packages the Agent Client Protocol JSON-RPC client, subprocess wrapper, session config, tool stream adapter, and update normalizer. Opencode and Cursor both build their runtimes on top of it, each layering a small provider-dialect module for its own extensions: Opencode's permission-mapping helpers, and Cursor's `cursorAcpExtensions` (`cursor/ask_question`, `cursor/create_plan`, `cursor/update_todos`).
...
| **providers/cursor** | Cursor Agent adaptor over first-party ACP (`agent acp`, not the retired `cursor-agent` stream-json CLI path) | Runtime (`AcpClientConnection` session lifecycle over the shared `src/providers/acp/` stack), `cursorAcpExtensions` dialect handlers (blocking `cursor/ask_question`, `cursor/create_plan`, `cursor/update_todos`), tool-name normalization, prompt encoding, JSONL history hydration, settings reconciliation, plan-path conventions, settings UI |
