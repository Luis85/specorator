...
- First run: every provider ships `enabled: false`, so a fresh install opens the **Setup view** (`src/features/onboarding/`) once — it probes each provider's CLI through that provider's own `ProviderCliResolver`, offers a shell-free in-app install for what is missing (`ProviderRegistration.cliInstall`), and confirms the vault folders, chat defaults, and placement. Gated on `firstRunDismissed` + no enabled provider, opened from the deferred (`onLayoutReady`) onload path.
