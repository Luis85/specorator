...
- **MVP graph constraints:** exactly one source node, exactly one sink node, single linear path, no cycles, no isolated nodes. Branching DAGs, conditionals, loops, and sub-workflows are explicit follow-up REQs.
- **Scratch dir layout:**
  ```
  specs/workflow-runs/
