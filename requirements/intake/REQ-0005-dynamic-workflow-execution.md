...
acceptance_criteria:
  ...
  - "Validation errors (`InvalidSlashCommand`, `InvalidWorkflowRunsFolder`, `CanvasNotFound`, `CanvasParseError`, `MalformedCanvas`, `UnsupportedCanvasNode`, `SkillNotFound`, `InvalidSkillFormat`, `UnsupportedGraph`, `CycleDetected`) and runtime errors (`StepOutputMissing`, `AgentExecutionFailed`, `Cancelled`) are returned as `Result.error` and surfaced via `NotificationPort.showError` naming the offending node or step. Non-SKILL canvas nodes (any node that is not a `type === 'file'` node pointing to a `.md` file) MUST trigger `UnsupportedCanvasNode` and abort the run; duplicate node ids and edges referencing missing nodes MUST trigger `MalformedCanvas`; the parser MUST NOT silently drop or coerce malformed entries."
  - "A new sidebar slash command `/workflow \"<canvas-path>\" <input>` is the only invocation surface in MVP. Grammar: the literal token `/workflow`, then the canvas path — wrapped in straight double quotes if it contains any whitespace, otherwise unquoted; quotes are stripped before vault resolution and embedded `\\\"` escapes a literal quote — then a single space, then the rest of the message verbatim as `<input>` (including embedded spaces, newlines, and quotes; no further parsing). The parser MUST reject malformed invocations with an explicit error naming the problem — missing canvas argument, unterminated quoted path, missing input, or canvas path resolving outside the vault — instead of best-effort splitting on whitespace. Auto-detection of canvas attachments in chat messages is explicitly out of scope and tracked as a follow-up REQ."
  - "A new setting `workflowRunsFolder` (default `specs/workflow-runs`) is added to `PluginSettings` and consumed by the use case."
  - "Before any vault write, the use case MUST validate `workflowRunsFolder` using the vault-path safety check (`normalizeVaultPath`). Values that are empty, absolute, contain `..` traversal, or target a reserved vault root (e.g. `.obsidian`) MUST fail with `InvalidWorkflowRunsFolder { value, reason }` returned as `Result.error` with no side effects — no folder or file is created."
...
- **Invocation surface:**
  - **MVP:** `/workflow "<canvas-path>" <initial-input>` slash command in the sidebar. Quoted-path grammar handles the common case of Obsidian paths containing spaces (e.g. `"My Notes/Refactor Flow.canvas"`); everything after the closing quote and a single space is the literal `<input>`. Unquoted paths are accepted only when they contain no whitespace. Parser unit tests cover: unquoted simple path, quoted path with spaces, escaped quotes inside the path (`\"`), unterminated quote, missing canvas, missing input, multi-line input.
