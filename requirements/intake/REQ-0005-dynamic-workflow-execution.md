...
acceptance_criteria:
  ...
  - "Validation errors (`CanvasNotFound`, `CanvasParseError`, `UnsupportedCanvasNode`, `SkillNotFound`, `InvalidSkillFormat`, `UnsupportedGraph`, `CycleDetected`) and runtime errors (`StepOutputMissing`, `AgentExecutionFailed`, `Cancelled`) are returned as `Result.error` and surfaced via `NotificationPort.showError` naming the offending node or step. Non-SKILL canvas nodes (any node that is not a `type === 'file'` node pointing to a `.md` file) MUST trigger `UnsupportedCanvasNode` and abort the run; the parser MUST NOT silently drop them."
