...
| ADR-SS-001 | Split env-snippet persistence — the NON-SECRET structure (`EnvSnippetStruct`: id/name/description/scope/non-secret `EnvEntry[]`/`contextLimits`) device-local via `SettingsPort` (additive OPTIONAL `PluginSettings` fields, mirroring the P9 `homeFsConsent` round-trip in `_coerceSettings`), the SECRET-bearing values via `SecretStorePort` under the deterministic `env.<scope>.<KEY>` namespace with the structure holding only a `secretRef`; a PURE classifier (regrown `providerEnvironment.ts` — shared-known/provider-owned/shared-unknown) decides secret-vs-non-secret; injection reuses the P9 runtime env merge; NO new port (compose `SettingsPort` + `SecretStorePort` behind a pure `EnvSnippetService`, ADR-008); NO plaintext secret in `data.json`/device-local; no new consent gate (reuse `isAvailable()`) | **Accepted** — P10 (autonomous-drive; ratifies CLAR-SS-001 + CLAR-SS-004; parallels ADR-PV-002) |
| ADR-SS-002 | Drive the settings shell from a PURE `buildSettingsViewModel(settings, registry, getCatalog, secretKeysSet, …)` → `SettingsViewModel` (ordered sections: shared/core first then enabled providers in blank-tab order, each with only the controls it SUPPORTS) — capability-gated by the frozen bag, NEVER `switch(providerId)` (extends ADR-PV-001 §4); keep the `PluginSettingTab` as Obsidian `Setting`-API DOM (NOT Vue, CLAR-SS-002/NG2), coverage-excluded `src/plugin/**` with manual legs; sections surface their existing P6–P9 ports; safe-DOM + no blocking dialogs; native a11y keyboard nav (WCAG 2.2 AA) | **Accepted** — P10 (autonomous-drive; ratifies CLAR-SS-002; realises NFR-SS-003/008/011) |
