...
| **Cursor** | Yes | `.cursor/plans` | Sets the ACP `plan` session mode. When the agent calls the blocking `cursor/create_plan` request, the plan text renders and the shared in-turn plan card opens — the request is answered `accepted`/`rejected`/`cancelled` from your decision, and the agent continues (or stops) within the same turn. A plan turn that produces only plain assistant text instead reports `planCompleted: true` and opens the post-turn approval card. |
