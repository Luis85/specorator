...
export class InlinePromptController {
  ...
  async handleExitPlanMode(
    ...
    // Cursor supplies the plan inline via cursor/create_plan (no plan FILE), so
    // fall back to input.plan when there's no file content to preview/approve.
    const inlinePlan = typeof input.plan === 'string' && input.plan ? input.plan : null;
    const planContent = content ?? inlinePlan;
