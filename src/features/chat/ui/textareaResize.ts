...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.specorator-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--specorator-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--specorator-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--specorator-textarea-min-height': `${minHeight}px`,
    '--specorator-textarea-max-height': `${maxHeight}px`,
  });
}
