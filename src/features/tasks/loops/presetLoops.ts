...
export const PRESET_LOOPS: SaveLoopInput[] = [
  ...
  },
  {
    ...
    approach: 'Refactor until the architecture reaches its stated target, preserving observable behavior at every step. Move in small reversible increments — extract and consolidate over rewrite. After each significant step, live-test the system, run an independent review, and commit. Track progress in a scratch file.',
    steps: '1. Write down the architectural target, constraints, and current risks before editing code; if coverage is thin where you will work, add characterization tests first.\n2. Make one significant, reviewable, behavior-preserving change at a time — never mix a behavior change into the refactor.\n3. Live-test the affected behavior and run an independent review after each significant step; existing tests should stay green and untouched.\n4. Commit each verified checkpoint and update the progress file with decisions, blockers, and the next action.',
    verify: 'The architecture is satisfactory and checks pass; observable behavior is unchanged; each significant step was live-tested, reviewed, and committed.',
    notes: 'Define what "satisfactory" means before starting — module boundaries, dependency direction, passing tests, and acceptable performance. Prefer pure functions and focused modules over stateful god-objects; leave a clone alone when a shared abstraction would be harder to read than the duplication.',
  },
  {
