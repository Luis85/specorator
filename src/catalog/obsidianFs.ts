...
export function obsidianFs(app: App): FileSystem {
  ...
  return {
    async read(p) { const n = normalizePath(p); return (await a.exists(n)) ? a.read(n) : null; },
    async write(p, c) { await a.write(normalizePath(p), c); },
    async exists(p) { return a.exists(normalizePath(p)); },
    async remove(p) { const n = normalizePath(p); if (await a.exists(n)) await a.remove(n); },
