...
function deserializeFeature(content: string): Feature | null {
  ...
  if (!data.id || !data.title || !data.slug || !data.status || !data.currentStep) return null
...
export class FeatureRepository implements IFeatureRepository {
  ...
  private metaPath(slugValue: string): string {
    return `${this.settings.featuresFolder}/${slugValue}/${META_FILE}`
  }

  async findAll(): Promise<Feature[]> {
    const folders = await this.bridge.listFolders(this.settings.featuresFolder)
    const features = await Promise.all(
      folders.map(async (folder) => {
        const path = `${this.settings.featuresFolder}/${folder}/${META_FILE}`
        try {
  ...
  async save(feature: Feature): Promise<Result<void>> {
    try {
      const folder = `${this.settings.featuresFolder}/${feature.slug.toString()}`
