...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('specorator-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'specorator-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'specorator-fork-target-option', text: label });
