...
- Command discovery differs by provider
  ...
  - Cursor skill discovery comes from `CursorSkillCatalog` (read-only `SKILL.md` scans of Cursor-native roots); it has no runtime command discovery
...
- Subprocess env: **all four** providers build their child env via `buildFullSubprocessEnvironment` (`providers/subprocessEnvironmentAllowlist.ts`) — Cursor, Opencode and Codex for their own spawns, and Claude for the `Options.env` it hands the SDK (persistent, cold-start, and the command probe). It spreads the **full** `process.env` then the resolved custom env because a GUI-launched host (Obsidian/Electron) otherwise hands its CLI child an impoverished environment whose own shell tool can't resolve host binaries (git/node "not on PATH"). The prior allowlist was removed 2026-07-13 so no provider hits a missing-var/PATH problem the Claude integration doesn't.
  - The **denylist** kill-switch (`NODE_TLS_REJECT_UNAUTHORIZED`, case-insensitive) is the one hard filter and still applies to host AND custom env.
  - PATH is overridden with the enhanced path and collapsed to a single canonical key (Windows env names are case-insensitive; shipping both `Path` and `PATH` could let the child resolve the un-enhanced one).
  - The enhanced path itself is built from `pickEnvValueCaseInsensitive(customEnv, 'PATH')`, never `customEnv.PATH`: a provider Environment entered as `Path=` IS the user's PATH on Windows, and an exact-key read would silently drop their tool dirs. Claude read the exact key until 2026-07-25 — which also made the onboarding probe (case-insensitive from the start) report a CLI as ready that Claude's own spawn then rejected with `missing-node`. Detection and spawn now read the same answer by construction.
  - Provider API keys and MCP secrets live in Obsidian SecretStorage, not the process env; the residual exposure of the full spread is other host env vars reaching the (user-installed) third-party CLI, matching how Claude already spawns.
