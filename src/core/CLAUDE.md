...
- Subprocess env: every provider that spawns a CLI subprocess (Cursor, Opencode, Codex) builds its child env via `buildFullSubprocessEnvironment` (`providers/subprocessEnvironmentAllowlist.ts`). It spreads the **full** `process.env` then the resolved custom env — Claude-parity (`{ ...process.env, PATH: enhancedPath }`) — because a GUI-launched host (Obsidian/Electron) otherwise hands its CLI child an impoverished environment whose own shell tool can't resolve host binaries (git/node "not on PATH"). The prior allowlist was removed 2026-07-13 so no provider hits a missing-var/PATH problem the Claude integration doesn't.
