...
/**
 * The stdio (or stdio-like) byte streams a JSON-RPC client rides on. The
 * provider adapters supply these — e.g. a subprocess's stdout/stdin plus an
 * `onClose` hook fed by process exit — so the client itself stays transport- and
 * provider-agnostic.
 */
...
/**
 * Newline-delimited JSON-RPC 2.0 client over a pair of byte streams (ADR-0001
 * Move 2). Owns the request/response correlation map, request timeouts and
 * abort, server-notification and server-request routing, line framing, and
 * pending-request rejection on close/dispose. Provider-native protocol shapes
 * stay in the adapters that wrap it.
 */
