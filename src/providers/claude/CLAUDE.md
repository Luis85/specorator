...
- **Effective-source gate.** Discovery (skills AND agents) scans `PluginManager.getEffectivelyEnabledPlugins()`, not `getPlugins()`: a plugin is surfaced only when it's enabled via a setting source the runtime actually reads. `PluginManager` keeps the per-source `enabledPlugins` state (`enableSources` over `.claude/settings.local.json`, `.claude/settings.json`, and `~/.claude/settings.json`) and `ClaudeWorkspaceServices` injects a resolver reporting the live effective sources (`resolveClaudeSettingSources` over `loadUserSettings` + the vault-trust gate). `isPluginEffectivelyEnabled` is the **sole** discovery authority — it walks the sources in the runtime's merge precedence (**local > project > user**), consulting only effective ones, so a plugin enabled only through `~/.claude/settings.json` while `loadUserSettings` is off — or through project/local on an untrusted vault — is NOT shown as a runnable `/<plugin>:<skill>` the runtime would drop, while a plugin disabled in project but enabled in an effective lower-precedence source the runtime does load is NOT wrongly vetoed. It keeps the legacy default-on only when a plugin is unmentioned by every source (missing settings). Raw `getPlugins()` (full-precedence merge, default-on) still returns the unfiltered set for the settings toggle UI.
- **Manifest path overrides.** `SkillStorage` reads each plugin's `.claude-plugin/plugin.json` and honors custom `skills` directory paths (string or array). Per the runtime, these are **additive**: the default `skills/` is always scanned and manifest dirs load alongside it (deduped by name, default wins). Manifest paths are third-party, so they're sanitized — absolute/`~`/`..`-traversal paths are rejected before joining against the install path.
