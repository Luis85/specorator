...
- **Effective-source gate.** Discovery (skills AND agents) scans `PluginManager.getEffectivelyEnabledPlugins()`, not `getPlugins()`: a plugin is surfaced only when it's enabled AND enabled via a setting source the runtime actually reads. `PluginManager` keeps the per-source `enabledPlugins` state (`enableSources`) and `ClaudeWorkspaceServices` injects a resolver reporting the live effective sources (`resolveClaudeSettingSources` over `loadUserSettings` + the vault-trust gate). So a plugin enabled only through `~/.claude/settings.json` while `loadUserSettings` is off — or through project settings on an untrusted vault — is NOT shown as a runnable `/<plugin>:<skill>` the runtime would silently drop. Raw `getPlugins()` still returns the unfiltered set for the settings toggle UI. `isPluginEffectivelyEnabled` keeps the legacy default-on only when a plugin is unmentioned by every source (missing settings), so the common case never regresses.
