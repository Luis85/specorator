...
import { mergeCursorTodosFromRaw, type NormalizedTodo } from './cursorTodoMerge';
...
export function registerCursorAcpExtensions(
  ...
  const applyTodoUpdate = (params: unknown): unknown[] => {
    ...
    // A late update_todos after Stop (per-turn signal aborted) must not refresh
    // the todo panel on the cancelled turn — mirror the ask/create_plan guard.
    // Ack the RPC with the raw todos so the agent isn't left hanging; skip the
    // emit and the cache write so the cancelled turn leaves no residue.
    if (host.getAskSignal?.()?.aborted) {
      return rawTodos;
    }
