...
import type { AssetMeta } from "../catalog/types";

export interface ConsentSummary {
  paths: string[]; requires: string[]; scanFlagged: boolean; body: string;
}

export function buildConsentSummary(
  asset: AssetMeta, paths: string[], scanFlagged: boolean
): ConsentSummary {
  return { paths, requires: asset.requires, scanFlagged, body: asset.body };
}

export class ConsentModal extends Modal {
  constructor(app: App, private summary: ConsentSummary, private onConfirm: () => void) {
    super(app);
  }
  onOpen() {
    ...
    if (this.summary.scanFlagged)
      contentEl.createEl("p", { text: "⚠ Content scan flagged this asset — review carefully.", cls: "mod-warning" });
    contentEl.createEl("pre", { text: this.summary.body });
    const btn = contentEl.createEl("button", { text: "Confirm install" });
    btn.addEventListener("click", () => { this.onConfirm(); this.close(); });
  }
