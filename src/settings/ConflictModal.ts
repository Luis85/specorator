...
export class ConflictModal extends Modal {
  ...
  onOpen(): void {
    ...
    const choose = (c: ConflictChoice) => () => { this.onChoice(c); this.close(); };
    contentEl.createEl("button", { text: "Keep mine (skip)" })
      .addEventListener("click", choose("skip"));
    contentEl.createEl("button", { text: "Backup & replace" })
      .addEventListener("click", choose("backup"));
    contentEl.createEl("button", { text: "Overwrite" })
