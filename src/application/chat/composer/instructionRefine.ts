/**
 * Pure instruction-refine transforms (SPEC-CP-015, ADR-CP-003, REQ-CP-016). The
 * prompt + parse functions are ported VERBATIM from claudian-main
 * `core/prompt/instructionRefine.ts`. Both are pure and total — they never throw
 * and have no side effects (NFR-CP-005). No `obsidian`/Vue import; the application
 * layer imports domain only.
 */

/** The parsed outcome of a refine side-query response (SPEC-CP-015). */
export type RefineOutcome =
	| { kind: 'refined'; instruction: string }
	| { kind: 'clarification'; question: string };

/** Ported verbatim from claudian-main `core/prompt/instructionRefine.ts`. */
...
const INSTRUCTION_BLOCK = /<instruction>([\s\S]*?)<\/instruction>/;

/**
 * Parse a refine response (ported from claudian `parseInstructionRefineResponse`,
 * mapped to the SPEC-CP-015 union): an `<instruction>…</instruction>` block →
 * `{kind:'refined'}`; otherwise a non-empty plain text → `{kind:'clarification'}`;
 * an empty/whitespace response → `null` (the use case maps this to `Result.err`).
 * Pure/total.
 */
export function parseRefineResponse(raw: string): RefineOutcome | null {
	const match = INSTRUCTION_BLOCK.exec(raw);
	if (match) {
		return { kind: 'refined', instruction: match[1].trim() };
	}
