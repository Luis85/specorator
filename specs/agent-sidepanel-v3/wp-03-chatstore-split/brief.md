...
**New:**
- `src/ui/stores/chatThreadsStore.ts` — owns `chatThreads`, `activeThreadId`, `markThreadUsed`, `captureSessionId`, persistence keys. Persisted lifecycle.
- `src/ui/stores/streamingTurnStore.ts` — owns `streamingText`, `streamingThinking`, `streamingToolCalls`, `lastUsage`, `cliStartingUp`, `sessionResumed`, plus actions `appendStreamingDelta`, `appendStreamingThinking`, `startStreamingToolCall`, `appendStreamingToolCallInput`, `finishStreamingToolCall`, `setLastUsage`, `setCliStartingUp`, `setSessionResumed`, `resetStreaming`. Per-turn lifecycle.
- `src/ui/stores/proposalStore.ts` — owns `proposals`, `addProposal`, `setProposalStatus`, `clearThreadProposals`. Cross-cutting lifecycle.
- `src/ui/stores/messagesStore.ts` — owns the per-thread message log AND the per-panel chat-I/O surface (everything left after the three lifecycles above): `messages` (per-thread map), `compactBoundaries` (per-thread map), `contextFiles`, `effectiveContextFiles`, `userText`, `response`, `status`, `errorType`, `truncated`, `structuredFail`, plus actions `appendMessage`, `clearThreadMessages` (drops messages + compactBoundaries for the thread — single owner; do not duplicate on `chatThreadsStore`), `appendCompactBoundaryNotice`, `addContextFile`, `removeContextFile`, `setActiveFile`, `setUserText`, `beginRequest`, `setResponse`, `setError`, `clearResponse` (also resets `structuredFail`), `setStructuredFail`. Per-panel UI lifecycle.
  - Rationale: `compactBoundaries` is thread-scoped transcript metadata read per `threadId` in `MessageList.vue` and **must survive `resetStreaming()`** on transport-change / new-conversation paths — that's why it lives here, not in `streamingTurnStore`. Bundling the chat-I/O surface (`userText` / `response` / `status` / context files) with `messages` keeps consumers from depending on five stores for what is one panel concern.
