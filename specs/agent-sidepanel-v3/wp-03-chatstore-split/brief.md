...
**New:**
- `src/ui/stores/chatThreadsStore.ts` — owns `chatThreads`, `activeThreadId`, `markThreadUsed`, `captureSessionId`, persistence keys.
...
- `src/ui/stores/messagesStore.ts` — owns `messages` (per-thread map), `appendMessage`, `clearThreadMessages`. (Single owner; do not duplicate the action on `chatThreadsStore`.)
