...
- **Fixed in this PR — deprecated `KeyboardEvent.keyCode === 229` IME-guard fallback** in `InlinePlanApprovalCard.vue:107` + `ChatInput.vue:245`. Replaced with the spec-canonical `if (event.isComposing) return;`. Obsidian's Electron baseline (the primary surface) is spec-compliant and honors `isComposing` throughout composition. CJK input on the standalone-web Safari demo is an explicit **non-goal** — see `docs/non-goals.md` and PR [#395](https://github.com/Luis85/specorator/pull/395) Codex threads (won't-fix per maintainer decision). No deprecated APIs, no `eslint-disable`, no tracking refs, no workarounds.
