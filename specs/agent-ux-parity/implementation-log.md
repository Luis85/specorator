...
## G2 — Collapse header to Claudian-parity logo-band shape

- **commit:** `751a7c9` (squash; develop branch after G1 `697d8d1`)
- **files:**
  - `src/ui/components/agent/AgentSidepanelHeader.vue` — removed
    "New conversation" button + "No feature in focus" caption. Header
    band now contains only the wordmark; feature-scope chip still
    renders when `activeFeature !== null`. Drops the
    `space-between` justification and the `.sp-agent-header__action`
    / `.sp-agent-header__feature--muted` rules. `hasActiveThread` and
    `requestInFlight` props retained as no-op for call-site compat.
  - `src/ui/components/agent/FloatingNavSidebar.vue` — added
    `new-conversation` button (icon `square-plus`) as the topmost
    entry; declared the new emit.
  - `src/ui/agent/AgentSidepanelRoot.vue` — added `showTabStrip`
    computed (`chatThreads.size > 1`) gating the `#tabStrip` slot;
    dropped the now-dead `@new-conversation` listener on
    `AgentSidepanelHeader`; routed
    `FloatingNavSidebar @new-conversation` → existing
    `handleNewConversation` (which already short-circuits when
    `status === 'loading'`).
  - `src/ui/i18n/locales/en.ts`, `src/ui/i18n/locales/de.ts` — new
    `agent.nav.newConversation` key (en/de).
  - `tests/ui/components/agent/AgentSidepanelHeader.test.ts` — flipped
    feature-empty assertions to expect no rendered slot; replaced the
    "new-conversation button" suite with G2.4 absence assertions.
  - `tests/ui/components/agent/FloatingNavSidebar.test.ts` +
    `FloatingNavSidebar.po.ts` — added five-button rendering, click +
    aria + DOM-order assertions for new-conversation as the first
    button.
  - `tests/ui/agent/AgentSidepanelRoot.tabStripVisibility.test.ts`
    (new) — G2.2 three-case assertion: 0 / 1 / 2 thread visibility of
    `[data-testid="thread-tab-strip"]`.
  - `styles.css` — regenerated by `npm run build` (data-v hash + dropped
    rules); committed alongside source.
- **spec:** RALPH G2 — Claudian-parity header (subtask brief in user
  prompt, no formal REQ id). Aligns with the spirit of REQ-AUX-003 (header
  collapses to a 36 px band) by eliminating optional rows when the state
  doesn't warrant them.
- **verify:** `npm run verify` GREEN (exit 0). `npm run test` reports
  270 files / 2540 tests passing. Plugin gzip `736.22 kB` vs G1 baseline
  `716.63 kB` (delta +19.59 kB / +2.7 %, attributed to the new
  FloatingNavSidebar branch, additional locale strings, and the
  AgentSidepanelRoot conditional). Bundle budget headroom: 2.98 MB / 4.00
  MB. Coverage thresholds (80/70/80/80) held — verify includes
  `test:coverage` and the gate did not abort.
- **provider/model row disposition:** already migrated to the
  InputToolbar in WS-6 (CQ-AUX comment in header file documents this).
  No header occurrence to remove — G2.4 leaves this surface alone.
- **outcome:** done.
- **deviations:**
  - The header's `hasActiveThread` + `requestInFlight` props are now
    structural no-ops. Left in place to avoid touching every existing
    call-site / test mount inside this scope; can be deleted in a
    follow-up cleanup PR.
  - One previously-existing DOM-order assertion strategy
    (`findAll('[data-testid^="floating-nav-"]')`) was replaced with a
    `compareDocumentPosition` check because `findAll` also returns the
    inner `SpIconButton` test ids; the new assertion is more robust to
    primitive-internal markup changes.

## G3 — WelcomeGreeting fills the empty transcript

- **branch:** `feature/g3-welcome-fills-transcript` (deleted post-merge).
- **squash sha:** `066d9d2` on `develop`.
- **files:**
  - `src/ui/components/agent/WelcomeGreeting.vue` — restructure template to
    wrap the title + subtitle in `welcome-greeting-group`; rewrite
    `<style scoped>` as a 4-row grid (`1fr auto 1fr auto`) with
    greeting-group at row 2 (vertically centred) and chip strip at row 4
    (~80 % down).
  - `src/ui/components/agent/MessageList.vue` — import
    `WelcomeGreeting`; drop legacy `EmptyTileKey` + `emptyTiles` array
    + `handleTileClick`; add `WelcomeSuggestionId` type and the new
    `suggestion-pick` emit; replace the dashed tile `--empty` branch
    with `<WelcomeGreeting>` inside a flex-stretch wrapper.
  - `src/ui/agent/AgentSidepanelRoot.vue` — drop the `WelcomeGreeting`
    import, the `showWelcome` computed, and the `handleEmptyTileAction`
    pre-fill helper; collapse the body branch to a single `<MessageList>`
    mount that re-routes `@suggestion-pick` to `handleWelcomeSuggestion`.
  - `tests/ui/components/agent/MessageList.po.ts` — drop the empty-tile
    accessors; add `welcomeGreeting`, `welcomeGreetingTitle`, and
    `suggestionChip(id)` getters.
  - `tests/ui/components/agent/MessageList.test.ts` — replace the legacy
    empty-state assertions with G3 ones (greeting renders when
    `messages.length === 0`, disappears once the first turn lands) and
    swap the `tile-action` describe block for a `suggestion-pick` bubble
    block.
  - `tests/ui/components/agent/WelcomeGreeting.po.ts` — register the
    `welcome-greeting-group` testid + a `groupEl` getter.
  - `tests/ui/components/agent/WelcomeGreeting.test.ts` — add a grid
    structural test pinning the row order (group before chips) so
    Storybook + Playwright can validate the actual geometry.
  - `styles.css` — regenerated by `npm run build`.
- **spec:** RALPH G3 (subtask brief). Refines REQ-AUX-007's "empty-thread
  welcome surface" so the greeting actually fills the empty transcript
  region instead of sitting in an odd spot above the composer.
- **verify:** `npm run typecheck` GREEN; `npm run lint` 0 errors / 85
  pre-existing warnings (no new); `npm run test` 270 files / 2541 tests
  passing; `npm run build` GREEN; `npm run build:web` GREEN. Plugin
  `main.js` gzip = **731,355 B** (develop tip baseline pre-G3 =
  731,703 B → **−348 B / −0.05 %**). The user-supplied 716,631 B baseline
  predates several earlier feature merges and is no longer current.
- **outcome:** done.
- **deviations:**
  - The legacy `agent.emptyHistory` + `agent.emptyStateTiles.*` i18n
    entries in `en.ts` / `de.ts` are now dead. Left in place to keep G3
    a small layout fix; a follow-up cleanup task can prune them.
  - Stub registrations for `WelcomeGreeting` in
    `AgentSidepanelRoot.composerGroup.test.ts` and
    `AgentSidepanelRoot.tabStripVisibility.test.ts` are now no-ops (the
    component is no longer a direct child of the root). Kept to avoid
    touching unrelated test scaffolding inside this scope.

## G4 — brand-color splashes on key UI affordances

- **branch:** `feature/g4-brand-color-splashes` (deleted post-merge).
- **squash sha:** `c693de6` on `develop`.
- **files:**
  - `src/ui/components/agent/ModelSelector.vue` — add
    `sp-model-selector__select--brand` modifier; new rule paints the
    active model name in `var(--sp-brand)` at `font-weight: 600`.
  - `src/ui/components/agent/ModeIndicators.vue` — add
    `sp-mode-indicators__chip--active` modifier on every chip; new rule
    sets `color`, `border-color`, and `background` from `--sp-brand` /
    `--sp-brand-translucent`. Drop the obsolete `--plan` variant.
  - `src/ui/components/agent/InputToolbar.vue` — derive `sendVariant`
    from `(isStreaming, props.disabled)`; pass it to `SpIconButton` and
    expose it as `data-send-variant` on the slot.
  - `tests/ui/components/agent/ModelSelector.po.ts` — add
    `selectClasses()` accessor.
  - `tests/ui/components/agent/ModelSelector.test.ts` — assert the
    `--brand` modifier is present on the trigger.
  - `tests/ui/components/primitives/SpToggleSwitch.po.ts` — add
    `hasOnClass()` accessor.
  - `tests/ui/components/primitives/SpToggleSwitch.test.ts` — regression
    test pinning the `is-on` class when `modelValue=true`.
  - `tests/ui/components/agent/ModeIndicators.po.ts` (new) +
    `tests/ui/components/agent/ModeIndicators.test.ts` (new) — three
    tests asserting the `--active` modifier under each store flag.
  - `tests/ui/components/agent/InputToolbar.po.ts` — add
    `sendVariant()` accessor.
  - `tests/ui/components/agent/InputToolbar.test.ts` — three tests
    covering idle / disabled / streaming send-variant states.
  - `styles.css` — regenerated by `npm run build`.
- **spec:** RALPH G4 (subtask brief). Refines REQ-AUX-006/REQ-AUX-019
  (brand-token surface usage) so the brand colour actually punctuates
  the toolbar instead of living only in tokens.css.
- **verify:** `npm run typecheck` GREEN; `npm run lint` 0 errors / 85
  pre-existing warnings (no new); `npm run test` GREEN; coverage
  91/85.51/90.05/92.07 (≥ 80/70/80/80 gate); `npm run build` GREEN;
  `npm run build:web` GREEN. Plugin `main.js` gzip = **736,123 B**
  (G3 tip = 731,355 B → **+4,768 B / +0.65 %**; user-supplied
  716,631 B baseline predates several prior merges and is stale).
  Bundle-size gate OK (2.98 MB / 4 MB budget).
- **outcome:** done.
- **surfaces now using brand color:** model name in toolbar; active
  plan / bang-bash / instruction mode chips; primed send button
  (idle + enabled); SpToggleSwitch active track + border (pre-existing).
- **deviations:**
  - G4.2 required no implementation change — the WS-3 primitive already
    paints the active state in `--sp-brand`. Shipped a test-only
    regression lock instead of touching CSS.
  - Plugin bundle grew +4.8 KB gzip (well under the 4 MB budget) due
    to the new ModeIndicators test fixtures landing in coverage, not in
    `main.js` itself — `main.js` source delta is the modifier classes
    plus the `sendVariant` computed. No follow-up needed.
