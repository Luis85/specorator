...
| **R-AUX-04** | MINOR | `MessageList.vue` (REQ-AUX-014) | Per-message avatars / model name / timestamp are rendered inline in `MessageList.vue` rather than in the extracted `MessageItem.vue` originally specced. test-report §2 marks this PASS *(inline render)*. The behaviour is correct and tested; the refactor was deferred to retrospective follow-up. | Acceptable for this feature. Carry forward as a "MessageItem extraction" retrospective task; do not block release. |
...
| **MessageItem.vue extraction** (REQ-AUX-014) | See R-AUX-04 — behaviour shipped inline; refactor is a follow-up. |
...
| REQ-AUX-014 | `MessageList.vue` (inline) | `MessageList.test.ts` | SATISFIED (inline render — see R-AUX-04) |
