...
1. **R-CP-001** — wire `customSystemPrompt` to the CLI (`--append-system-prompt`) **or** record an
   explicit spec re-scope to P5. Verify on the real CLI (TEST-CP-M2).
2. **R-CP-002** — bind the inline-block channel to the per-tab runtime + enqueue the request chunks
   in the streaming consumer **or** record an explicit spec re-scope to P5 with an ADR. The
   "lights up with no UI change" promise must be made true or withdrawn.
