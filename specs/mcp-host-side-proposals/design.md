...
flowchart TB
  ...
  subgraph plugin ["Specorator plugin (Obsidian process)"]
    ...
    store["ProposalStore (extended)<br/>ephemeral queue, per-id mutex,<br/>EventBus emitter"]
    eventBus["EventBus<br/>proposalEnqueued / proposalDecided"]
...
| `ObsidianMcpServerAdapter` | infrastructure | Owns the HTTP loopback (ADR-013) and the `initialize`-handshake hook that populates `McpClientIdentifier`; injects the `EventBus` into `ProposalStore`; wires the new tool registrars; the orphaned `acceptProposal`/`rejectProposal`/`getProposals` shims become thin delegates to the new store methods with hard-coded `decisionBy = "user"` for the sidepanel-card path (REQ-MHP-039, REQ-MHP-040). | The per-request `McpServer` factory. | All of the above. | **MODIFIED** — `src/infrastructure/obsidian/ObsidianMcpServerAdapter.ts`. |
| `FileWriteProposalCard.vue` | UI (Vue) | Existing card. Now subscribes (via its existing Pinia store) to `proposalDecided` events for its own `proposalId`; on external decision, transitions to the existing `accepted`/`rejected` terminal state with the S24 "Decided in `<client.id>`." note. Cross-surface invariant from Part A §F3. | Its own render branches (unchanged structurally). | `EventBus` (via the existing Pinia proposal store). | **MODIFIED** — additive only. |
| `EventBus` | shared | Plain typed pub/sub. Carries `proposalEnqueued`, `proposalDecided` (plus `client.id` for the decider). Single bus per plugin instance. | The subscriber list. | None. | **NEW** — `src/infrastructure/events/ProposalEventBus.ts` (or extends an existing project-wide bus if one is available; the architect notes none is in the current `src/` tree per `CLAUDE.md` §Key files, so a feature-local module is acceptable). |
...
| `invalid_argument` | Escape-hatch arg fails regex / traversal check | `obsidian_cli_read_command` | REQ-MHP-013 |
...
| REQ-MHP-041 | Arch §"Components and responsibilities" (`ActiveFeatureResolver` referenced from `ProposalStore` data flow); Arch §"Data flow" F1 step 4; Arch §"Key decisions" (Auto-accept active-slug resolution row) |
...
| REQ-MHP-046 | UX §F7; UI §S10–S16 (status-bar states + pending-proposal Notice states); Arch §"Components and responsibilities" (`StatusBarBadge`, `ProposalNoticeEmitter`); Arch §"System overview" (event-bus path) |
