...
| 7 — Implementation | in-progress | `implementation-log.md` | WS-1 (rename), WS-2 (`ProviderSelection` + migration), and WS-3 (TransportSelector reshape + ProviderRegistry wiring) complete on branches `feature/mps-ws-1-rename-port`, `feature/mps-ws-2-provider-selection`, and `feature/mps-ws-3-selector-wiring`. WS-4..WS-9 fan-out unlocked; WS-10 integration waits on the fan-out. |
| 8 — Testing | pending | — | |
| 9 — Review | pending | — | |
| 10 — Release | pending | — | |
...
| 2026-05-21 | dev (WS-8) | dev (WS-10 integration) | WS-8 complete on branch `feature/mps-ws-8-status-modes-model-attach` (cut from WS-3 tip `b579a9f`; commits `1930c1a`, `[sub-batches 3-5]`, `df4a21f`). Sub-batches: (1) Status panel — `statusPanelStore` + `StatusPanel.vue` + `TodoList.vue` + `BashHistoryList.vue`; StreamDelta extended additively with `tool-result`, `todo-update`, `citation`. (2) Modeline modes — `chatInputModeStore` + `ModeIndicators.vue` + ChatInput Shift+Tab/`!`/`#` wiring + ChatTurnOrchestrator `planMode`/`instructionSuffix`/`model`/`attachments` forwarding + `buildSubprocessArgs` `--permission-mode plan`. (3) Slash-command provider enrichment in `useSlashPalette`. (4) Model selector + ProviderBadge/Menu + `chatProviderStore` (registry-validated) + perf test on a 100-message thread (NFR-MPS-004 ≤ 200 ms). (5) Attachments — `attachmentsStore.add` returns `Result<void, ChatTransportError>` rejecting > 5 MB with `ATTACHMENT_TOO_LARGE` + `AttachmentStrip.vue` paste/drag-drop. **Wiring hand-off:** `TurnInputBuilder` does NOT yet snapshot the three new stores into `TurnInput` — orchestrator already accepts the fields. That snapshot wiring + replacing the temporary WS-3 stub `degradedClaudeCliPort` slots is WS-10 integration. Tests: 95+ new pass; full UI suite green except a small set of integration tests that need similar stub treatment to `AgentSidepanelRoot.slashCommands.test.ts` (mock `StatusPanel` / `AttachmentStrip` / `ProviderBadge` / `ModelSelector` in mounts that don't supply `PROVIDER_REGISTRY_KEY`). |
