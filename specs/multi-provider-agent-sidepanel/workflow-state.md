...
| 7 — Implementation | in-progress | `implementation-log.md` | WS-1 (rename), WS-2 (`ProviderSelection` + migration), WS-3 (TransportSelector reshape + ProviderRegistry wiring), and WS-5 (Cursor CLI adapter + binary resolver) complete on branches `feature/mps-ws-1..3-*` and `feature/mps-ws-5-cursor-cli`. WS-4, WS-6..WS-9 still in fan-out; WS-10 integration waits on the fan-out. |
| 8 — Testing | pending | — | |
| 9 — Review | pending | — | |
| 10 — Release | pending | — | |
