...
| 7 — Implementation | in-progress | `implementation-log.md` | WS-1 (rename), WS-2 (`ProviderSelection` + migration), WS-3 (TransportSelector reshape + ProviderRegistry wiring), WS-4 (Cursor API adapter + secret storage + settings UX), and WS-5 (Cursor CLI adapter + binary resolver) complete on `feature/mps-ws-1..5-*` branches. WS-6..WS-9 fan-out in flight; WS-10 integration waits on the fan-out. |
| 8 — Testing | pending | — | |
| 9 — Review | pending | — | |
| 10 — Release | pending | — | |
...
| 2026-05-21 | dev (WS-4) | dev (WS-5..WS-9 fan-out) | WS-4 complete on branch `feature/mps-ws-4-cursor-api` (cut from WS-3 tip `b579a9f`; commits `a40a0c3` → `e2fefb5` → `c9607d7` → `5bbfb2a` → `74d9af1` → `98f3f30` → `edc3114`). ADR-MPS-003 filed; CQ-MPS-01 closed-as-deferred via the research spike (`research-cursor-api.md`) — Cursor publishes no versioned third-party SSE endpoint yet, adapter encoded against design §C8 with placeholder base URL `https://api.cursor.sh/v1` and `cursorApiPreview` defaulting to `false`. `SECRET_ID_CURSOR` constant added to `SecretStorePort`. Full `CursorApiAdapter` with late key read, SSE parser (`message_delta` / `tool_use` / `usage` / `done` / `error` + §10 unexpected-close recovery), 5 MB attachment cap, and no-key-in-logs discipline (NFR-MPS-002 verified by `CursorApiAdapter.logging.test.ts`). `MockCursorApiAdapter` test fake with the Claude-mock configuration surface (NFR-MPS-014). `CursorKeyField.vue` two-variant settings component with co-located PageObject. `CursorSettingsSection.ts` mounts the field + `cursorApiPreview` toggle + `autoPreferProvider` dropdown into the Obsidian settings tab. `main.ts` hydrates `_cursorKeyCache` alongside `_apiKeyCache`, constructs the real `CursorApiAdapter` in `onload()`, and projects `cursorApiKeyPresent` from the cache so the selector now lights up Cursor API when the preview flag and key are both set. End-to-end leakage test (`cursor-key-leakage.test.ts`) confirms NFR-MPS-001 — the canary key value never appears in the simulated `data.json` blob. `npm run verify` green: 1992 / 1992 unit tests across 173 files, plugin bundle 2.93 MB / 4 MB, standalone 0.27 MB / 2 MB, typedoc clean, npm audit clean. **Conflict-zone for WS-5 merge:** `src/plugin/main.ts` swaps `_cursorCliStub` for the real `CursorCliAdapter` and projects `cursorCliResolved`; the seam in `_routeTransport` is already prepared. Next ready task: **T-MPS-058** (WS-5 first task, per `dispatch-plan.md`). |
