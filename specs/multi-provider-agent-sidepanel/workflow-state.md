...
| 7 — Implementation | in-progress | `implementation-log.md` | WS-1 (rename), WS-2 (`ProviderSelection` + migration), WS-3 (TransportSelector reshape + ProviderRegistry wiring), WS-6 (multi-thread switcher), and WS-7 (per-message Copy / Regenerate / Edit-and-resend) complete on branches `feature/mps-ws-1-rename-port`, `feature/mps-ws-2-provider-selection`, `feature/mps-ws-3-selector-wiring`, `feature/mps-ws-6-multi-thread`, and `feature/mps-ws-7-message-actions`. WS-4 / WS-5 / WS-8 / WS-9 fan-out still in flight; WS-10 integration waits on the fan-out. |
| 8 — Testing | pending | — | |
| 9 — Review | pending | — | |
| 10 — Release | pending | — | |
...
| 2026-05-22 | dev (WS-7) | dev (WS-4 / WS-5 / WS-8 / WS-9 + WS-10 integration) | WS-7 complete on branch `feature/mps-ws-7-message-actions` (cut from WS-6 tip `89ea04f`; commits `4065879` → `17aaae7` → `9ec2e27` → `0cde417` → closeout). Three workstreams in one branch: (1) `MessageActions.vue` with Copy / Regenerate / Edit affordances (REQ-MPS-026/027/028), Regenerate hidden for non-latest assistant, both Regenerate + Edit disabled while streaming (REQ-MPS-029), aria-labels per action (NFR-MPS-008); (2) `messagesStore.removeLatestAssistant()` + `truncateAfter(index)` truncation primitives; (3) wired via `MessageList` re-emit + `ChatSidebar.expose({ copyMessageToClipboard, regenerateMessage, editMessage })` + `AgentSidepanelRoot` routing — mirrors the existing `tile-action` chain. Regenerate re-dispatches the prior user prompt through the unmodified `handleSend()` pipeline so the orchestrator's `resume_session_id` semantics (WS-6) keep the SDK session continuous. Edit truncates the trailing turns and prefills the textarea — user adjusts and presses Enter. **Verification:** `npm run test` green (2017 tests across 178 files); `npm run typecheck` clean; `npm run lint` 0 errors. Next ready tasks remain in WS-4 / WS-5 / WS-8 / WS-9; WS-10 integration still gates on full fan-out completion. |
