...
import { computeBadge, DEFAULT_PLATFORMS } from "../../src/settings/CatalogSettingsTab";
...
describe("computeBadge", () => {
  ...
  it("'Update available' when installed but the catalog version is newer", () => {
    expect(computeBadge({
      installed: true, requiresOk: true, installedHash: "old", catalogHash: "new",
  ...
  it("'Needs tool (denied)' when a required tool is present but deny-moded (v0.1.0)", () => {
    expect(computeBadge({ installed: false, requiresOk: false, denied: true })).toBe("Needs tool (denied)");
  });
