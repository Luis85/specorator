import type { FileSystem } from "../../src/catalog/types";
...
export function memFs(seed: Record<string, string> = {}): MemFs {
  ...
  return {
    ...
    async write(p, c) {
      if (failPaths.has(p)) { failPaths.delete(p); throw new Error(`simulated write failure: ${p}`); }
      store.set(p, c);
      for (const d of ancestors(p)) dirSet.add(d);
    },
    async exists(p) { return store.has(p) || dirSet.has(p); },
    async remove(p) { store.delete(p); dirSet.delete(p); },
    async mkdirp(p) { for (const d of ancestors(p + "/_")) dirSet.add(d); },
