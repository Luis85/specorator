...
import { enableAsset, disableAsset, ScanBlockedError } from "../../src/catalog/installer";
import { loadState } from "../../src/catalog/sidecar";
import { memFs } from "./memfs";
import type { AssetMeta } from "../../src/catalog/types";

const asset: AssetMeta = {
  id: "auditing-vault", name: "auditing-vault", description: "d", type: "skill",
...
describe("installer", () => {
  ...
  it("installs a shared dependency exactly once across the order", async () => {
    ...
    const root: AssetMeta = { ...asset, id: "auditing-vault", name: "auditing-vault", dependsOn: ["scanning-links"] };
  ...
  it("rolls back the asset's file when the record write fails", async () => {
    ...
    await expect(enableAsset(fs, asset, [asset], ["claude"])).rejects.toThrow(/simulated write failure/);
