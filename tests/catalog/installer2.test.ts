...
import { enableAsset, disableAsset, ConflictError, ScanBlockedError } from "../../src/catalog/installer";
import { loadState } from "../../src/catalog/sidecar";
import { parse as parseYaml } from "yaml";
import { GEMINI_MANIFEST_PATH } from "../../src/catalog/gemini";
import { memFs } from "./memfs";
import type { AssetMeta } from "../../src/catalog/types";

const cmd: AssetMeta = {
  id: "audit-vault", name: "audit-vault", description: "d", type: "command",
  version: "0.1.0", bundle: "Vault Audit", requires: [], dependsOn: [], body: "# Run audit",
};
...
describe("installer (phase 2)", () => {
  it("writes a command to claude + gemini with correct formats", async () => {
    ...
    expect(await fs.read(".gemini/extensions/specorator/commands/audit-vault.toml")).toContain("prompt =");
    ...
    expect(state["audit-vault"].paths).toContain(".gemini/extensions/specorator/commands/audit-vault.toml");
  ...
  it("R2: hard-blocks hidden-unicode content before any write (ScanBlockedError)", async () => {
    ...
    await expect(enableAsset(fs, evil, [evil], ["claude"]))
  ...
  it("H5: routes an untracked-file conflict through onConflict (overwrite)", async () => {
    ...
    await enableAsset(fs, cmd, [cmd], ["claude"], {
      onConflict: async (p) => { seen.push(p); return "overwrite"; },
    });
  ...
  it("H5: still throws ConflictError when no onConflict callback is supplied", async () => {
    const fs = memFs({ ".claude/commands/audit-vault.md": "USER" });
    await expect(enableAsset(fs, cmd, [cmd], ["claude"]))
