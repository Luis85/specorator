...
describe('NavigationSidebar', () => {
  ...
  describe('previous/next message navigation', () => {
    function addUserMessage(el: MockElement, offset: number): MockElement {
      const msg = el.createDiv({ cls: 'specorator-message specorator-message-user' });
      msg.offsetTop = offset;
      return msg;
    }

    function addAssistantMessage(el: MockElement, offset: number): MockElement {
      const msg = el.createDiv({ cls: 'specorator-message specorator-message-assistant' });
      msg.offsetTop = offset;
      return msg;
    }
    ...
    function getButtons(parent: MockElement) {
      const container = parent.querySelector('.specorator-nav-sidebar')!;
      return {
  ...
  describe('destroy', () => {
    it('should remove container from DOM', () => {
      ...
      expect(parentEl.querySelector('.specorator-nav-sidebar')).not.toBeNull();

      sidebar.destroy();

      expect(parentEl.querySelector('.specorator-nav-sidebar')).toBeNull();
    });
  });
});
